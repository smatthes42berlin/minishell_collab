...
int						append_token_list(t_main_data *main_data,
							t_token *token);
...
int						create_empty_token(t_main_data *main_data,
							t_token *token);
...
void					init_hdoc_info(char *delim_raw,
...
int						parent_get_str_child(t_list_d **hdoc_op_token,
							int fd[2], int pid, t_here_doc_info *hdoc_info);
int						child_read_hdoc(t_here_doc_info *hdoc_info, int fd[2]);
int						add_heredoc_str_token(t_list_d **hdoc_op_token,
							char *res, t_here_doc_info *hdoc_info);
...
int						check_new_root_node(t_parse_info *parse_info,
							t_node *new_node);
int						got_to_nth_next_token(int num,
							t_parse_info *parse_info);
int						set_n_token_as_parsed(int num,
							t_parse_info *parse_info);
...
int						check_cmd_access(char **env_vars, char *cmd_arg,
							char **exec_path);
int						check_path_combination(char *cur_path_val,
							char *cmd_arg, char **exec_path);
...
/* test enviroment */
// -> test cmd!
// t_node_exec				*test_cmd_exec(char *command, char *flag,
// 							bool is_inbuilt);
