...
/*   Updated: 2024/01/22 18:27:36 by rkost            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_node	*set_cmd_2(void)
{
	...
	ret = malloc_handler(sizeof(t_node));
	ret->node_type = test_cmd_pipe("Pipe 1", EXEC, EXEC, test_cmd_exec("sleep",
				"/bin/sleep", "5"), test_cmd_exec("ls", "/bin/ls", "-l"));
	ret->type = PIPE;
	return (ret);
}
...
t_node	*set_redir_out_1(void)
{
	...
	ret->node_type = test_cmd_redir("redir", "out", FILE_ONLY_WRITE_APPEND,
			STDOUT, EXEC, test_cmd_exec("ls", "/bin/grep", "nn"));
	ret->type = REDIR;
	return (ret);
}
