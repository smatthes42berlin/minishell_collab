...
static char *checking_exit(t_node_exec *exec_node,  int *exit_code)
{
    ...
    else if (exec_node->argv[0][0] == '.')
    {
        ...
        {
            ret = use_strjoin(exec_node->argv[0], ": Permission denied", "use_strjoin");
            *exit_code = 127;
        }
    }
    ...
    {
        ret = use_strjoin(exec_node->argv[0], ": command not found", "use_strjoin");
        *exit_code = 127;
    }
    return (ret) ;
}
...
static char *checking_path(t_node_exec *exec_node,  int *exit_code)
{
    ...
    {
        ret = use_strjoin(exec_node->argv[0], ": Is a directory", "use_strjoin");
        *exit_code = 126;
    }
    else
    {
        ...
        {
            ret = use_strjoin(exec_node->argv[0], ": No such file or directory", "use_strjoin");
            *exit_code = 127;
        }
    }
    return (ret) ;
